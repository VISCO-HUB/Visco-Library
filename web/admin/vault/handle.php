<?php
	...
	SWITCH($QUERY) {				
		...
		BREAK;
		CASE 'TAGSREFRESH': ECHO $ISSUPERADMIN ? TAGS::REFRESH($DATA) : $NORIGHTS;
		BREAK;
		// MAIL
		CASE 'SENDEMAIL': ECHO $ISSUPERADMIN ? EMAILSYSTEM::SENDMAIL($DATA) : $NORIGHTS;
		BREAK;
		// DASHBOARD
		CASE 'DASHBOARDINFO': ECHO $ISADMIN ? DASHBOARD::INFO() : $NORIGHTS;
		BREAK;
		CASE 'DOWNLOADLOGGET': ECHO $ISADMIN ? DASHBOARD::DOWNLOADLOG($DATA) : $NORIGHTS;
		BREAK;
		// COMMENTS
		CASE 'COMMENTSGET': ECHO $ISADMIN ? COMMENTS::GET($DATA) : $NORIGHTS;
		BREAK;
		CASE 'COMMENTSDEL': ECHO $ISADMIN ? COMMENTS::DEL($DATA) : $NORIGHTS;
		BREAK;
		// PROD			
		...
		BREAK;
		CASE 'PRODDELETE': ECHO $ISADMIN ? PRODUCTS::PRODDELETE($DATA) : $NORIGHTS;
		BREAK;
		// MSG
		CASE 'MSGGETCNT': ECHO $ISADMIN ? MSGSYSTEM::GETCNT() : $NORIGHTS;
		BREAK;
		CASE 'MSGGET': ECHO $ISADMIN ? MSGSYSTEM::GET($DATA) : $NORIGHTS;
		BREAK;
		CASE 'MSGSETPARAM': ECHO $ISADMIN ? MSGSYSTEM::SETPARAM($DATA) : $NORIGHTS;
		BREAK;
		CASE 'MSGDELETE': ECHO $ISADMIN ? MSGSYSTEM::DEL($DATA) : $NORIGHTS;
		BREAK;
		//USERS
