	...
	<div ng-repeat="prod in products.products" class="col-xxlg-2 col-xlg-3 col-lg-4 col-md-6 col-sm-6 col-xs-6 col-xxs-12 text-center flex">
		<div class="card col-xxs-12 col-xs-12 resp">
			<div class="card-content col-xxs-12 col-xs-12 disable-hover-xxs">
				<div class="text-center margin-bottom-5 relative"> <a href="#prod/{{prod.id}}"><img ng-src="{{getProdImages(prod.previews, 1, true)}}" class="text-center col-xxs-12"></a>
					...
					<a href="admin/#/models-edit/{{prod.id}}/1" class="btn btn-default btn-danger button-fixed button-admin-edit" uib-tooltip="Edit" ng-show="auth.rights>=1"> &nbsp;&nbsp;</a> </div>
				...
				<div class="card-hidden visible-xxs">
					...
					<table width="100%" border="0">
						<tr>
							<td><div class="btn-group dropup btn-block margin-0" tooltip-placement="right" ng-show="auth.browser=='MXS'">
									<button type="button" class="btn btn-primary" style="width: 80.5%"  ng-click="placeModel(prod.id)"> 
										{{placename}} 
									</button>
									<button type="button" class="btn btn-primary dropdown-toggle width-20" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <span class="caret"></span> <span class="sr-only">Toggle Dropdown</span> </button>
								...
								<button type="button" ng-click="downloadUrl(prod.id)" class="btn btn-block margin-0" ng-show="auth.browser!='MXS'" ng-class="{'disabled': auth.rights < 1, 'btn-danger': modelError[prod.id], 'btn-primary': !modelError[prod.id]}"> {{modelError[prod.id] ? 'System Error!' : 'Download'}} </button></td>
							<td width="34px"><button type="button" class="btn btn-default custom-button-gray btn-block button-fixed button-heart">&nbsp;</button></td>
						</tr>
					</table>
				</div>
			</div>
